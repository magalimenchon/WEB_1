...
function iniciarPagina(){
    "use strict";
    let apiURL = 'https://web-unicen.herokuapp.com/api/';
let baseURL = apiURL + 'groups/';
...
function getInformationByItem(){
  ...
  fetch("https://web-unicen.herokuapp.com/api/thing/" + item, {
    ...
    .then(function(response){
    if( ! response.ok ){
       document.querySelector("#infoItem").innerHTML = "Error " + response.status
    } else {
      return response.json();
    }
  }).then(function(resultData){
       //al ser tipo JSON resultData es un objeto listo para usar
        ...
        html += "Informacion: " + JSON.stringify(resultData.information['thing']) + "<br />";
...
function guardarInformacion(){
  ...
  if (grupo && informacion){
    ...
    .then(function(response){
      if( ! response.ok ){
       document.querySelector("#guardarAlert").innerHTML = "Error " + response.status
      } else {
        return response.json();
      }
    })
...
console.log(item)
  if (item){
    ...
    .then(function(response){
      if( ! response.ok ){
       document.querySelector("#deleteInfoItem").innerHTML = "Error " + response.status
      } else {
        return response.json();
      }
    })
